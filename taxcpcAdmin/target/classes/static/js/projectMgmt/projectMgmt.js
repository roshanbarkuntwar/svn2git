...
//	document.getElementById("parent_menu_code").disabled = false;
//	document.getElementById("sub_menu_type").disabled = false;
...
//		parentmenuName.append('<option value="">--Select--</option>');
//		submenuName.append('<option value="">--Select--</option>');

		$.each(response, function(k, v) {
			moduleName.append('<option value=' + k + '>' + v + '</option>');
//			parentmenuName.append('<option value=' + k + '>' + v + '</option>');
//			submenuName.append('<option value=' + k + '>' + v + '</option>');
...
function getProjectList(){
  ...
  if (!lhsIsNull(projectCode)) {
		...
		$('#parent_menu_code').val("");
		//$('#sub_menu_type').val("");
		 $("#sub_menu_type").multiselect("clearSelection");

		var url = "./getProjectList?projectCode="+projectCode;
		ajaxPostUrl(url, getProjectListResponse);
	} else {
		// getProjectModulesResponse();
	}
}

function getProjectListResponse(response){
	var parentmenuName = $('#parent_menu_code');
	//var submenuName = $('#sub_menu_type');
...
if (!lhsIsNull(response)) {
		...
		parentmenuName.append('<option value="">--Select Parent MenuName--</option>');
		//submenuName.append('<option value="">--Select--</option>');
...
function getProjectSubmenuList(){
	
	var projectCode = $('#project_code').val();

	 $('#sub_menu_type').multiselect('enable');
	 ...
	 if (!lhsIsNull(projectCode)) {
			...
			var url = "./getProjectsubMenuList?projectCode="+projectCode;
			ajaxPostUrl(url, getProjectSubListResponse);
		} else {
			// getProjectModulesResponse();
		}
}
...
function getProjectSubListResponse(response){
	
	var sub_menu_type = $('#sub_menu_type');
	
	sub_menu_type.empty();
	if (!lhsIsNull(response)) {

		$.each(response, function(k, v) {
			sub_menu_type.append('<option value=' + k + '>' + v + '</option>');
		...
		$('#sub_menu_type').multiselect({
			...
			nonSelectedText: '--Select Submenu Name--'
		});
	} else {
		sub_menu_type.append('<option value="">--Select --</option>');
	}
}

function getApplicationTypeName(){
	var application_type = $('#application_type').val();
	document.getElementById("architecture_type_code").disabled = false;
	if(!lhsIsNull(application_type) && application_type ==='apptype_01' ){
		$('#architecture_type_code').val("");
		 $('#frontend_type_code').multiselect('enable');

		var url = "./getArchitecture_type?application_type=" + application_type;
	
	}else if(!lhsIsNull(application_type) && application_type ==='apptype_02'){
		$('#architecture_type_code').val("");
		 $('#frontend_type_code').multiselect('disable');
		var url = "./getArchitecture_type?application_type=" + application_type;

	}else if(!lhsIsNull(application_type) && application_type ==='apptype_03'){
		$('#architecture_type_code').val("");
		 $('#frontend_type_code').multiselect('enable');
	...
	ajaxPostUrl(url, getArchitectureResponse);

}

function getArchitectureResponse(response) {
	var projectArchitectureList= $('#architecture_type_code');
	projectArchitectureList.empty();
	
	if (!lhsIsNull(response)) {
		
		projectArchitectureList.append('<option value="">--Select Architecture --</option>');
		$.each(response, function(k, v) {
			projectArchitectureList.append('<option value=' + k + '>' + v + '</option>');
			// console.log("key " + k + "value " + v);
		});
	} else {
		projectArchitectureList.append('<option value="">--Select Architecture --</option>');
...
function validateProjectEntryForm(){
    ...
    var svn_link = $('#svn_projectname').val();
    
    var war_filename = $('#war_filename').val();
    
    var other_war_filename = $('#other_war_filename').val();
    ...
    if(lhsIsNull(project_name)) {
    	...
    	openErrorModal("Project Name Exceeds Maximum Limit Of Characters.");
        status = false;
    }else if(lhsIsNull(project_info)) {
    	openErrorModal("Please Enter Project Information.");
        status = false;
    } else if(project_info.length > 500) {
    	openErrorModal("Project Information Exceeds Maximum Limit Of Characters.");
        status = false;
		...
		status = false;
	} else if (svn_link.length > 500) {
		openErrorModal("SVN_link Exceeds Maximum Limit Of Characters.");
		status = false;
	}else if (war_filename.length > 500) {
		openErrorModal("War Filename Exceeds Maximum Limit Of Characters.");
		status = false;
	}else if (other_war_filename.length > 500) {
		openErrorModal("Other War Filename Exceeds Maximum Limit Of Characters.");
		status = false;
...
function validateModuleEntryForm(){
   ...
   // alert("action_name"+action_name);
    ...
    if(lhsIsNull(module_name)) {
        ...
        status = false;
    } else if(module_name.length > 100) {
    	openErrorModal("Module Name Exceeds Maximum Limit Of Characters.");
        status = false;
...
function validateMenuEntryForm(){
   ...
   if (lhsIsNull(project_name)) {
        ...
        status = false;
    } else if(menu_name.length > 100) {
    	openErrorModal("Menu Name Exceeds Maximum Limit Of Characters.");
        status = false;
    }else if(menu_description.length > 500) {
    	openErrorModal("Menu Description Exceeds Maximum Limit Of Characters.");
        status = false;
    }else if(menu_type.length > 200) {
    	openErrorModal("Menu Type Exceeds Maximum Limit Of Characters.");
        status = false;
...
function searchProjectDetailFilter()
{
	...
	if (lhsIsNull(projname) && lhsIsNull(status) && lhsIsNull(parent_code)) {
        openErrorModal("Please select any filter to see the records !");
	}else{

	var actionUrl = "./getSearchProjectDetail?projname=" +projname+ "&status=" +status+"&parent_code="+parent_code;
...
function searchProjectMenuDetailFilter(){
	...
	if (lhsIsNull(menuname)&& lhsIsNull(menuStatus) && lhsIsNull(project_name) && lhsIsNull(module_Name) ) {
        openErrorModal("Please select any filter to see the records !");
	}else{
		
		var url = "./getSearchProjectMenuDetail?menuname=" +menuname+"&menuStatus=" +menuStatus+"&project_name="+project_name+"&module_Name="+module_Name;
		//alert("url" +url);
...
function onload(){
	document.getElementById("architecture_type_code").disabled = true;
	
}


function getParentProjectInfo(inputElement) {
	var project_code = inputElement.value;
	if (!lhsIsNull(project_code)) {

		var url = "./getParentProjectInfo?project_code=" + project_code;
		ajaxPostUrl(url, getParentProjectInfoResponse);
...
function getParentProjectInfoResponse(listItem) {
 ...
 //  console.log(`${property}: ${map[property]}`);
	...
	for (var i = 0; i < listItem.length; i++) {
		...
		if (i == 0 && listItem[i] != null) {
			document.getElementById("application_type").value = listItem[i];
		} else if (i == 1 && listItem[i] != null) {
			document.getElementById("architecture_type_code").value = listItem[i];
		} else if (i == 2 && listItem[i] != null) {
			let itm = [];
			if (listItem[i].includes('#')) {
				itm = listItem[i].split('#');
				
				for (var j = 0; j < listItem[i].length; j++) {
					$('#frontend_type_code').val(itm);
					$("#frontend_type_code").multiselect("refresh");
				}
			} else {
				itm = listItem[i];
				$('#frontend_type_code').val(itm); 
		...
		} else if (i == 3 && listItem[i] != null) {
			let itm = [];
			if (listItem[i].includes('#')) {
				itm = listItem[i].split('#');
				for (var j = 0; j < listItem[i].length; j++) {
					$('#backend_type_code').val(itm);
					$("#backend_type_code").multiselect("refresh");
				}
			} else {
				itm = listItem[i];
				$('#backend_type_code').val(itm); 
		...
		} else if (i == 4 && listItem[i] != null) {
			let itm = [];
			if (listItem[i].includes('#')) {
				itm = listItem[i].split('#');
				for (var j = 0; j < listItem[i].length; j++) {
					$('#database_type_code').val(itm);
					$("#database_type_code").multiselect("refresh");
				}
			} else {
				itm = listItem[i];
				$('#database_type_code').val(itm); 
		...
		} else if (i == 5 && listItem[i] != null) {
			let itm = [];
			if (listItem[i].includes('#')) {
				itm = listItem[i].split('#');
				for (var j = 0; j < listItem[i].length; j++) {
					$('#framework_type_code').val(itm);
					$("#framework_type_code").multiselect("refresh");
				}
			} else {
				itm = listItem[i];
				$('#framework_type_code').val(itm); 
		...
		} else if (i == 6 && listItem[i] != null) {
			let itm = [];
			if (listItem[i].includes('#')) {
				itm = listItem[i].split('#');
				for (var j = 0; j < listItem[i].length; j++) {
					$('#project_type').val(itm);
					$("#project_type").multiselect("refresh");
				}
			} else {
				itm = listItem[i];
				$('#project_type').val(itm); 
		...
		} else if (i == 7 && listItem[i] != null) {
			document.getElementById("svn_link1").value = listItem[i];
		}else if(i == 8 && listItem[i] != null)	{
			document.getElementById("svn_projectname").value = listItem[i];
		} else if (i == 9 && listItem[i] != null) {
			document.getElementById("war_filename").value = listItem[i];
		} else if (i == 10 && listItem[i] != null) {
			document.getElementById("other_war_filename").value = listItem[i];
		} else if (i == 11 && listItem[i] != null) {
			document.getElementById("connected_db_user").value = listItem[i];
		} else if (i == 12 && listItem[i] != null) {
			document.getElementById("remark").value = listItem[i];
		} else if (i == 13 && listItem[i] != null) {
			console.log(listItem[i])
			if(listItem[i]=='A'){
				document.getElementById("project_status1").checked = true
			}else{
			document.getElementById("project_status2").checked = true
			}
		}
	}
