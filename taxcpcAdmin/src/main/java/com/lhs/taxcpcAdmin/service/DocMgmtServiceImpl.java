...
import com.lhs.taxcpcAdmin.dao.View_return_qt_Repository;
...
public class DocMgmtServiceImpl implements DocMgmtService {
	...
	@Autowired
	View_return_qt_Repository view_return;
	...
	@Override
	public List<View_return_qt> getListByYear() {
		List<View_return_qt> list = new ArrayList<>();

	
			try {
				
				String queryStr = "select distinct acc_year from VIEW_RET_QUT ";
				
				//list = view_return.findAll();
				list = executer.executeSQLQueryView_Ret(queryStr);
	...
	@Override
	public List<View_return_qt> getListByFYear() {
		List<View_return_qt> list = new ArrayList<>();

		
		try {
			
			//String queryStr = "select from_date, to_date,quarter,acc_year from view_ret_qut t  where t.return_type='FY'";
			
			String queryStr = "select from_date, to_date,quarter,acc_year from view_ret_qut t  where t.return_type='FY'";

			System.out.println("queryStr for FY...."+queryStr);
			//list = view_return.findAll();
			list = executer.executeSQLQueryView_Ret1(queryStr);
	...
	@Override
	public List<View_return_qt> getListByRegular_type() {
       List<View_return_qt> list = new ArrayList<>();

		
		try {
			
			String queryStr = "select from_date, to_date,quarter,acc_year from view_ret_qut t  where t.return_type='Regular'";
			
			//String queryStr = "select * from view_ret_qut t  where t.return_type='Regular'";

			
			System.out.println("queryStr for Regular...."+queryStr);
			//list = view_return.findAll();
			list = executer.executeSQLQueryView_Ret1(queryStr);
	...
	@Override
	public List<View_return_qt> getListBy15GH_type() {
		  List<View_return_qt> list = new ArrayList<>();

			
			try {
				
				//String queryStr = "select from_date, to_date,quarter,acc_year from view_ret_qut t  where t.return_type='15G/H'";
				
				String queryStr = "select * from view_ret_qut t  where t.return_type='15G/H'";

				System.out.println("queryStr for 15G/H...."+queryStr);
				//list = view_return.findAll();
				list = executer.executeSQLQueryView_Ret1(queryStr);
