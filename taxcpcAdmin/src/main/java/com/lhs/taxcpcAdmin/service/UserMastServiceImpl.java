	...
	public Map<String, String> getUserByDeptAndRole(String role_code) {
		...
		try {
			...
			if (role_code.equalsIgnoreCase("TSTR")) {
				...
				queryStr = "  select t.user_code, t.user_name from lhs_taxcpc_user_mast t, user_role_mast r WHERE "
...
//		System.out.println("dataGridDTO.getPaginator()..." + dataGridDTO.getPaginator());
		...
		try {
			String queryStr =
					"select t.user_code,\n" +
							...
							"         where t.role_code = v.role_code)||')' user_name from lhs_taxcpc_user_mast t\n" + 
	...
	public Map<String, String> getUserList() {
		Map<String, String> userList = new HashMap<>();
		try {
			String queryStr = "select user_code,user_name  from  lhs_taxcpc_user_mast";
	...
	public Map<String, String> getlist() {
		...
		try {
			String queryStr = "select user_code,favourite_menu from  lhs_taxcpc_user_mast ";
...
//	}
	...
	public int getcount(String user_name, String user_type, String user_mode, String role_code, String user_status) {
		...
		int count= 0;
		try {
			String Query = "select count(*) from lhs_taxcpc_user_mast where 1=1 ";
	...
	public List<UserMast> getSearchUserDetails(String user_name, String user_type, String user_mode, String role_code, String user_status) {
		...
		try {
			...
			Query = "select * from lhs_taxcpc_user_mast where 1=1 ";
