...
function validateLhssysDbDetailEntryForm(){
   ...
   // alert("action_name"+action_name);
    ...
    if(lhsIsNull(type_of_db)) {
     ...
     else if(lhsIsNull(db_username)) {
    	openErrorModal("Please Enter Database Username.");
        status = false;
    }else if(lhsIsNull(db_password)) {
    	openErrorModal("Please Enter Database Password.");
        status = false;
    }   
...
function deleteDatabaseRecord(){
	openConfirmModal("Do you want to delete ?", "deleteSelectedDatabaseRow()");
}



function deleteSelectedDatabaseRow() {
 closeConfirmModal();
 if(!lhsIsNull(checkedId)){
var split_id = checkedId.split('-');
var object_code = split_id[2];

var url = "./deleteDatabaseRow?object_code=" + object_code;
ajaxPostUrlWithParam(url, deleteDatabaseResponse);
}
}

function deleteDatabaseResponse(response) {
