...
function searchTreeFilter(){
	var entity_code = document.getElementById("entity_code").value;
	var formdata = $('#treeDetails').serializeArray();
	var fdata = JSON.stringify(formdata);

	if (lhsIsNull(entity_code)) {
		openErrorModal("Please select Entity Name to see the Tree Diagram!");
		// window.location.reload();
	}else{
		var actionUrl = "./TreeFilter?entity_code=" + entity_code;
		ajaxPostDataArray(actionUrl, fdata, searchFilterWorkResponse1);	
	}
			
}

function searchFilterWorkResponse1(response) {
	$('#dataSpan').html('');
