 ...
 * @author kapil.gupta
 */
public class SalarySevaarthProjectionDataGridAction extends ActionSupport implements SessionAware {
...
//        session.put("ERRORCLASS", MessageType.errorMessage);
        ...
        try {
            ...
            if (!utl.isnull(getFilterFlag()) && getFilterFlag().equalsIgnoreCase("true")) {
                if (!utl.isnull(getSearch())) {
                        ...
                        if (getRecordsPerPage() > 0) {
                            if (getPageNumber() > 0) {
                                ...
                                if (clientMast != null) {
                                    ...
                                    SalarySevaarthProjectionDB salDB = new SalarySevaarthProjectionDB();
                                    String l_query = salDB.getSalarySevaarthProjectionDataGridQuery(clientMast, asmt, getSalSevaarthProjectionFilterData(), minVal, maxVal);
                                    utl.generateSpecialLog("Salary Sevaarth Projection data query: ", l_query);
                                    DbGenericFunctionExecutor dbQueryList = new DbGenericFunctionExecutor();
                                    List<SalarySevaarthProjectionBrowseEntity> salSevaarthProjectionBrowseEntityList = dbQueryList.getGenericList(new SalarySevaarthProjectionBrowseEntity(), l_query);

                                    SalarySevaarthProjectionSupport salDg = new SalarySevaarthProjectionSupport();
                                    sb = salDg.getBrowseGrid(salSevaarthProjectionBrowseEntityList, clientMast, sb, getStartIndex(), getPageNumber());
                                } else {
                    ...
                    session.put("SALSEVAARTHPROJECTIONRECPERPG", String.valueOf(getSetRecPerPage()));
    ...
    public SalarySevaarthProjectionDataGridAction() {
        utl = new Util();
