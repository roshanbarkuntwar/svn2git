 ...
 * @author ayushi.jain
 */
public class ProcTdsCorrTranLoadUpdt {

    String proc_out_parameter = "0";//for return value if procedure already return some value the remove this
    Util utl;

    public String execute_procedure(final String entity_code, final String client_code, final String acc_year, final String assement_acc_year, final int quarter_no, final Date from_date, final Date To_date, final String tds_type, final long client_login_session_seq_no) {
            ...
            Work work = new Work() {
                ...
                public void execute(Connection cnctn) throws SQLException {
                    try {
                        ...
                        String executeProc = "{call LHS_TDS_IMP_CORR.PROC_TDS_CORR_TRAN_LOAD_UPDT(?,?,?,?,?,?,?,?,?,?)}";
                        ...
                        clst.registerOutParameter(10, java.sql.Types.CHAR);//register output parameter
                        clst.executeUpdate();
                        proc_out_parameter = clst.getString(10);
                        try {
                            clst.close();
    ...
    public ProcTdsCorrTranLoadUpdt() {
        utl = new Util();
    }
