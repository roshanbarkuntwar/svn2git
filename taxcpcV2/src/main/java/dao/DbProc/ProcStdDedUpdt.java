 ...
 */
public class ProcStdDedUpdt {

    String proc_out_parameter;
    Util utl;

    public ProcStdDedUpdt() {
        utl = new Util();
    }
        ...
        try {
            Work work = new Work() {
                ...
                public void execute(Connection cnctn) throws SQLException {
                    try {
                        utl.generateLog("***proc_stdded_updt parameters***", "");
                        ...
                        String executeProc = "{call lhs_hrd.proc_stdded_updt(?,?,?,?,?,?,?,?,?,?,?)}";
                        ...
                        clst.setNull(8, java.sql.Types.LONGVARCHAR);//rowid_seq
                        clst.setString(9, user_code);//user_code

                        if (proc_seq_no != null) {
                            clst.setLong(10, proc_seq_no);//proc_seq_no
                        } else {
                            clst.setLong(10, 0l);//proc_seq_no
