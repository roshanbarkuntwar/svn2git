...
public interface BankAuditAttachRepository extends JpaRepository<BankAuditAttach, Integer>,GenericCustomRepository<Integer, BankAuditAttach>{
	...
	@Query(value = "select doc_attach_name from LHS_TAXCPC_BANK_AUDIT_ATTACH where audit_mast_seq_id=:audit_mast_seq_id", nativeQuery = true)
	...
	@Query(value = "select doc_resolve_attach_name from LHS_TAXCPC_BANK_AUDIT_ATTACH where audit_mast_seq_id=:audit_mast_seq_id", nativeQuery = true)
