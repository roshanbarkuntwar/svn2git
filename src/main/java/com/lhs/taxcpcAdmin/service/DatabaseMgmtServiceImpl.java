	...
	public List<LhssysMainTables> getDatabaseTableDetailList() {
		List<LhssysMainTables> entity = new ArrayList<>();
			...
			try {
				...
				Query = "select DISTINCT t.table_or_view_name,\r\n" + 
						"       t.object_type,\r\n" + 
						"       t.remark,\r\n" + 
						"       t.lastupdate,\r\n" + 
						"       t.object_code,\r\n" + 
				...
				System.out.println("Query in db main caldssss------->"+Query);
	...
	public List<LhssysDbObjects> getSearchDatabaseUser(String db_object_name, String type_of_database, String status) {
		...
		String Query = "";
			...
			try {
                ...
                Query = "select DISTINCT * from lhs_taxcpc_db_objects where  object_type = 'USER' and";
...
//							: paginatorEntity.getTotal();
	...
	@Override
	public String deleteDatabaseRow(String object_code) {
		String response = "error";
		try {
			tablerepo.deleteById(object_code);
			response = "success";
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
