...
public class ReqTranLov {
	...
	public LovDataObject getParentReq() {
		...
		object.setQuery("select req_code, (select i.user_name from lhs_taxcpc_user_mast i where v.reported_by =  i.user_code)reported_by, reported_date, req_priority, req_type, issue_type, (select j.project_name  from project_mast j where j.project_code = v.project_name) project_name, issue_description from req_tran v order by reported_date desc");
