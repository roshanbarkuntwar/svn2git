...
function editProjectDictionary(id) {
	...
	if (!lhsIsNull(seq_id)) {
	   ...
	   var url = "./projectDictionaryEntry?seq=" + seq_id+"&action="+"edit";
		window.location =url;
...
function viewDictionary() {
	...
	var url = "./projectDictionaryEntry?seq=" +seq_id +"&action="+Action;
	window.location =url;
...
function onEditAction(){
	...
	if (action == "edit") {
		...
		document.getElementById('keyword_title_question').disabled= true;
